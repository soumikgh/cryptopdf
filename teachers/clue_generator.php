...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en" lang="en">
    <head>
        ...
        <script language="javascript" type="text/javascript">
			...
			var sample_text2=["","This is the place to<br />Have something to eat.<br />Bring your own lunch<br />Or get some other treat.","At this useful place<br />You do not need a cup.<br />Turn a handle to make<br />An arc of water come up.","They'll call your mom when<br />You go here for the clue<br />If the mouth stick says<br />One hundred and two.","To ride up and down,<br />Go in and close the door.<br />Push a number on a button<br />To stop at your floor."];
        ...
        Created by: Ilya Gerasimenko | http://www.gerasimenko.com/ */
            ...
            function updateStep(num)
				...
				if (show_step==false)
					...
					if (num==3)
						...
						{
						if (num==2 && locations_done==true && descriptions_done==true && title_selected==true)
							{
							show_step=true;
							}
						else
                ...
                if (show_step==true)
                    ...
                    if (currMenu==2)
                        ...
                        tempHTML+='	</tr>';
                        tempHTML+='	<tr>';
                        tempHTML+='		<td class="header_step2" style="padding:0px 2px 0px 2px;"><div class="helvetica11pxTanB">Clue #</div></td>';
                        tempHTML+='		<td class="header_step2R" style="padding:0px 2px 0px 2px;"><div class="helvetica11pxTanB">Location<br />Described</div></td>';
                        tempHTML+='		<td class="header_step2R" style="width:468px;"><div class="helvetica11pxTanB">Clue (Plaintext and CIPHERTEXT)</div></td>';
                        tempHTML+='		<td class="header_step2R" style="padding:0px 2px 0px 2px;"><div class="helvetica11pxTanB">Cipher</div></td>';
                        tempHTML+='		<td class="header_step2R" style="padding:0px 2px 0px 2px;"><div class="helvetica11pxTanB">Print Key?</div></td>';
                        tempHTML+='	</tr>';
                        ...
                        for (i=1; i<total_fields+1; i++)
                            ...
                            if (cipher_num[i]!=8)
								...
								if (cipher_num[i]==1 || cipher_num[i]==5 || cipher_num[i]==8)
									{
                              	  	tempHTML+='		<td class="col_stepR" style="width:468px;"><div class="clue_textS" style="width:468px;">'+converted_text+'</div></td>';
                               	 	tempHTML+='		<td class="col_stepR" align="center"><div class="arial14pxBlack">'+cipher_names[cipher_num[i]]+'</div><div>';
									}
								else
									{
                              	  	tempHTML+='		<td class="col_stepR" style="width:468px;"><div class="clue_textS" style="width:468px;">&#0160;'+converted_text.split("&#0160;").join("##").split("<br />").join("[[").split("").join(" ").split(" # #").join("&#0160;&#0160;").split(" ").join("&#0160; ").split("&#0160; [&#0160; [").join("<br />").split("<br />&#0160; ").join("<br />&#0160;")+'</div></td>';
                               	 	tempHTML+='		<td class="col_stepR" align="center"><div class="arial14pxBlack">'+cipher_names[cipher_num[i]]+'</div><div>';
									}
                                //paper code
                                if (i==1){tempHTML2+='<div class="arial14pxBlack">'+cipher_names[cipher_num[i]]+' Cipher,<br />';}
                            	}
                            else
                            	{//your own cipher...
							...
							if (cipher_num[i]==5)
								{
								tempHTML+='		<td class="col_stepR" align="center"><div class="arial14pxBlack">No Key</div></td>';
								}
                            else if (print_keys[i]==true)
                            	{
            ...
            function updateCipher(object_id,num,reset_obj)
				...
				if (print_keys[object_id]==true)
					{
					print_keys[object_id]=false;
					}
				else
					{
					print_keys[object_id]=true;
					}
				printKey(object_id);
			...
			function chooseSample(num,num2)
				...
				for (i=0; i<tempArray.length; i++)
					{
					tempArray[i]=tempArray[i].substring(0,26);
					}
				//somehow locate text fields that are empty to add them into.
				useRow=0;
				for (i=1; i<total_fields+1; i++)
					{
					if (document.formContainer['step1_description'+i+'_'+1]!=undefined)
						{
						if (document.formContainer['step1_description'+i+'_'+1].value=="" && document.formContainer['step1_description'+i+'_'+2].value=="" && document.formContainer['step1_description'+i+'_'+3].value=="" && document.formContainer['step1_description'+i+'_'+4].value=="")
							{
							useRow=i;
							break;
							}
				...
				//empty fields are being removed.
				if (useRow>0)
					{
					window['container_visible'+useRow]=["",false,false,false,false,false,false,false,false];
					for (i=0; i<tempArray.length; i++)
						{
						window['container_visible'+useRow][i+1]=true;
						}
					window['step1_descriptions'+useRow]=["","","","","","","","",""];
					//make row visible.
					for (j=1; j<window['container_visible'+useRow].length; j++)
						{
						if (window['container_visible'+useRow][j]==true) {document.formContainer['step1_description'+useRow+'_'+j].value=tempArray[j-1];}
						else {document.formContainer['step1_description'+useRow+'_'+j].value="";}
						if (window['container_visible'+useRow][j]==true) {showIt('container'+useRow+'_'+j);}
						else {hideIt('container'+useRow+'_'+j);}
						}
					document.formContainer['step1_location'+useRow].value=window['sample_titles'+num][num2];
					}
				else
					{
					if (total_fields>=8)
					...
					for (j=1; j<5; j++)
						{
						if (window['container_visible'+total_fields][j]==true) {document.formContainer['step1_description'+total_fields+'_'+j].value=tempArray[j-1];}
						else {document.formContainer['step1_description'+total_fields+'_'+j].value="";}
						}
            ...
            function saveStep1()
				...
				if (total_fields>0)
                	...
                	for (j=1; j<total_fields+1; j++)
						...
						if (document.formContainer['step1_location'+j]!=undefined)
							{
			...
			function deleteClue(num)
				{
				saveStep1();
				document.formContainer['step1_clue'+num].value=total_fields;
				if (total_fields>1)
					{
					for (i=num+1; i<total_fields+1; i++)
						{
						document.formContainer['step1_clue'+i].value=i-1;
						}
					}
				newVar=reorderList();
				total_fields-=1;
				updateList(total_fields);
				var element=document.getElementById('step1_num');
    			element.value=total_fields;
				saveStep1();
				if (total_fields<=0)
            ...
            function updateList(num)
				...
				if (num==0)
					{
					total_fields=num*1;
					document.getElementById('step1_fields').innerHTML="";
					}
                else
                	{
                    saveStep1();
                    ...
                    tempHTML='<div class="arial14pxBlackB">Now choose your hiding places and make plaintext clues to describe them. You can make your own clues or use some of our sample clues.</div>';
                    ...
                    for (i=1; i<total_fields+1; i++)
						...
						clue_order[i]=i;
                        tempHTML+='			<tr>';
                        tempHTML+='				<td class="col_step" align="center"><div style="padding-top:18px;"><input id="step1_clue'+i+'" name="step1_clue'+i+'" type="text" value="'+i+'" style="width:34px; height:16px; text-align:right; color:#000000;" class="form_field" onfocus="this.value=\'\';" onblur="checkClueNumDone(this,'+i+'); return false;" onkeyup="checkClueNum(this,'+i+'); return false;" maxlength="'+total_fields.toString.length+'" /></div><div style="margin-top:4px;"><a href="#" onclick="deleteClue('+i+'); return false;">[Delete]</a></div></td>';
                    ...
                    for (i=1; i<total_fields+1; i++)
                        ...
                        tempHTML+='		<td class="col_stepR" style="width:200px;"><div class="clue_text" id="step2_description'+i+'">'+step1_descriptions[i].split('\r').join('<br />')+'</div></td>';
						...
						if (print_keys[i]==true)
                        	{
                            tempHTML+='		<td class="col_stepR" align="center"><div id="print_container'+i+'"><a href="#" onclick="printKey('+i+'); return false;"><img id="step2_checkmark'+i+'" name="step2_checkmark'+i+'" src="../images/btn_checkmark_on.gif" width="18" height="19" alt="Checkmark" border="0" /></a></div><div style="display:none;" id="print_hidden'+i+'" class="arial14pxBlack">No Key</div></td>';
                        	}
                        else
                        	{
                            tempHTML+='		<td class="col_stepR" align="center"><div id="print_container'+i+'"><a href="#" onclick="printKey('+i+'); return false;"><img id="step2_checkmark'+i+'" name="step2_checkmark'+i+'" src="../images/btn_checkmark_off.gif" width="18" height="19" alt="Checkmark" border="0" /></a></div><div style="display:none;" id="print_hidden'+i+'" class="arial14pxBlack">No Key</div></td>';
                        	}
                    	}
                    tempHTML+='</table>';
            ...
            function printKey(num)
                ...
                if (document.images && (preloadFlag==true))
					...
					if (cipher_num[num]==5)
						{
						document.getElementById('print_container'+num).style.display="none";
						document.getElementById('print_hidden'+num).style.display="block";
						}
					else
						{
						document.getElementById('print_container'+num).style.display="block";
						document.getElementById('print_hidden'+num).style.display="none";
            ...
            function popupWait()
            	{
                tb_show("survey1","popup_wait.html?placeValuesBeforeTB_=savedValues&TB_iframe=true&height=140&width=260&modal=true");
                document.getElementById("TB_window").style.background="none";
            	}
	
            function popupReviewPrint()
            	{
                tb_show("survey1","popup_reviewprint.html?placeValuesBeforeTB_=savedValues&TB_iframe=true&height=140&width=260&modal=true");
                document.getElementById("TB_window").style.background="none";
            	}
	
            function popupPrinting()
            	{
                tb_show("survey1","popup_printing.html?placeValuesBeforeTB_=savedValues&TB_iframe=true&height=140&width=260&modal=true");
                document.getElementById("TB_window").style.background="none";
            	}
                ...
                if (document.formContainer.step4_name.value!="" && document.formContainer.step4_key.value!="")
                    ...
                    for (j=1; j<tempArray.length+1; j++)
                        ...
                        if (document.formContainer['step4_line'+j].value=="" || document.formContainer['step4_line'+j].value==your_own_type)
                        	{
							tb_show("survey1","popup_yourown.html?placeValuesBeforeTB_=savedValues&TB_iframe=true&height=140&width=260&modal=true");
                			document.getElementById("TB_window").style.background="none";
                            return;
                            break;
                        	}
			...
			currentMessage="Please use the links at the top of the page to go back. If you click Leave Page, you will lose any work you have done on your treasure hunt.";
                        ...
                        <div id="generator_content">
                            ...
                            <div id="content_step3" align="left">
                                <div class="floatL" id="hide4">
                                    ...
                                    <div class="arial14pxBlackB" style="margin-top:40px;">When you print your hunt, your printout will contain:</div>
