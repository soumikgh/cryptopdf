...
class Users {
    ...
    public function userLoginForm() {
        ...
        if ($form->isSubmitted() && $form->validate()) {
            ...
            $user = new User($this->db, getUserId() );
            $user->convertAnonDecryptions( $oldSessionId );
    ...
    public function userForgotPasswordForm() {
        ...
        $email = $form->addElement('text', 'email')->setLabel('Type your Email');
        ...
        $form->addElement('submit', 'submitForm', array('value' => 'Submit'))->setLabel('&nbsp;');
    ...
    public function userRegistrationForm() {
        $codenames = new Codenames($this->db);
        ...
        $codeNameHidden = $form->addElement('hidden', 'codenameCounter');
        $codeNameHidden->setId('codenameCounter');
        ...
        if ($form->isSubmitted() && $form->validate()) {
            el( $_SESSION );
            // @TODO: use codename and codename counter from session
            $this->save($_POST);
    ...
    public function save($data) {
        ...
        // save any anon activity
        $user = new User( $this->db, $userId );
        $user->convertAnonDecryptions(session_id() );
    ...
    public function setAuthCredentials($userId) {
        ...
        $_SESSION['auth']['codename'] = $user->codename;
        $_SESSION['auth']['encryptedCodename'] = $user->encryptedCodename;
    }
    ...
    private function sendUserCredentials($data) {
        ...
        $body .= 'Login Link: http://' . $_SERVER['HTTP_HOST'] . '/register.php';
    ...
    private function sendPassword($userId, $newPassword) {
        ...
        if (!empty($userData['email']) && check_email($userData['email'])) {
            ...
            $body = '
Your current username is: ' . $userData['username'] . PHP_EOL . '
Your new, temporary password is: ' .  $newPassword . PHP_EOL . '
To choose a different password, log in here with the temporary password.' . PHP_EOL . '
Login at: http://' . $_SERVER['HTTP_HOST'] . '/register.php' . PHP_EOL . PHP_EOL . '

Then change your password on the Account Profile page.' . PHP_EOL . '
_______' . PHP_EOL . '
The CryptoClub' . PHP_EOL . '
http://www.cryptoclub.org';

            if (!mail($userData['email'], $subject, $body)) {
                mail( 'msteudel@gmail.com', 'Crypto email problem', 'Unable to send email to: ' . $userData['email']);
            }
        }
    }
    ...
    public function logTime($userId) {
        if (!empty($userId ) ) {
            ...
            $this->user_model->logTime($userId, $sessionId);
        }
    }
    ...
    public function makeGroupAdmin($userId, $groupId) {
        if (is_numeric($userId) && is_numeric($groupId)) {
            $this->group_model->addGroupAdmin($userId, $groupId);
        }
    }

    public function removeGroupAdmin($userId, $groupId) {
        if (is_numeric($userId) && is_numeric($groupId)) {
            $this->group_model->removeGroupAdmin($userId, $groupId);
        }
    }
